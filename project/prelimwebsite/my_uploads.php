...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Popup messages - matching login_signup.php style */
        .message {
            position: fixed;
            top: 20px;
            left: 50%;
        ...
        @media (max-width: 992px) {
            ...
            .nav-link { width: 100%; text-align: center; padding: 12px !important; }
            .dropdown-menu { position: static; width: 100%; margin-top: 5px; box-shadow: none; background: rgba(255,255,255,0.9); }
        }
        ...
        @media (max-width: 768px) {
            ...
            .filter-btns { width: 100%; justify-content: center; }
            .preview-meta { flex-direction: column; align-items: center; }
            .list-content { flex-direction: column; align-items: flex-start; }
...
<body>
    ...
    $(document).ready(function() {
        function showToast(message, type) {
            type = type || 'success';
            $('.message').remove(); // remove any existing message
        ...
        function doDelete(resourceId) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var jsonStr  = jsonStart !== -1 ? trimmed.slice(jsonStart) : trimmed;
                    ...
                    if (result.success) {
                        // Close modal safely if it is open
                        try {
                        ...
                        $('.resource-card[data-id="' + resourceId + '"]').fadeOut(300, function() { $(this).remove(); });
                        $('.list-item[data-id="'     + resourceId + '"]').fadeOut(300, function() { $(this).remove(); });
        ...
        // Load resource details into modal
        function loadResourceDetails(resourceId) {
            var card = $('.resource-card[data-id="' + resourceId + '"]');
            if (!card.length) return;

            var resourceData = card.data('resource');
            ...
            $('#viewFileBtn, #downloadFileBtn, #deleteFileBtn').data('id', resourceId);

            var ft = resourceData.file_type.toLowerCase();
            if (ft === 'docx' || ft === 'ppt') {
                $('#viewFileBtn').hide();
        ...
        //  Card / list-item click → open modal
        $(document).on('click', '.resource-card, .list-item', function(e) {
            if (!$(e.target).closest('.action-btn').length) {
                loadResourceDetails($(this).data('id'));
            }
        });

        // View-resource button
        $(document).on('click', '.view-resource', function(e) {
            e.stopPropagation();
            var card = $(this).closest('.resource-card, .list-item');
            var ft   = card.find('.file-type-badge').text().toLowerCase().trim();
            if (ft === 'docx' || ft === 'ppt') {
                showToast('This file type cannot be viewed in the browser. Please use the download button instead.', 'warning');
                return;
            }
            loadResourceDetails($(this).data('id'));
        });

        // Download button (card/list)
        $(document).on('click', '.download-resource', function(e) {
            e.stopPropagation();
            window.location.href = 'download_resource.php?id=' + $(this).data('id');
        ...
        $('#viewFileBtn').click(function() {
            window.open('view_resource.php?id=' + $(this).data('id'), '_blank');
        });
        ...
        //  Disable view button for docx/ppt files in grid/list 
        $('.view-resource').each(function() {
            var card = $(this).closest('.resource-card, .list-item');
            var fileTypeBadge = card.find('.file-type-badge').text().toLowerCase().trim();
